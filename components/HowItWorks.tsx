      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {steps.map((step, i) => (
          <div key={step.num} className="relative bg-card-bg border border-border rounded-lg p-7 shadow-card hover:shadow-hover transition-shadow">
            ...
            {i < steps.length - 1 && (
              <div className="hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-warm-bg border border-border rounded-full items-center justify-center text-[11px] text-muted z-10">
                →
              </div>
            )}
          </div>
